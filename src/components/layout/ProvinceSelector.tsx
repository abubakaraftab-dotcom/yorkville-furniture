      ...
      {isOpen && (
        <div className="absolute right-0 top-full z-50 w-64 pt-2">
          <div className="rounded-xl border border-border bg-white p-2 shadow-xl" role="menu">
            <p className="px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted">Choose your delivery province</p>
            <p className="px-3 pb-2 text-[10px] leading-4 text-muted">Prices and availability update to match this province.</p>
            {availableProvinces.map((province) => (
              ...
              >
                ...
                {currentProvince?.code === province.code && <span className="ml-auto text-primary">✓</span>}
              </button>
            ))}
          </div>
        </div>
