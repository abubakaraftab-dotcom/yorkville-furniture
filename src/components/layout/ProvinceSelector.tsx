...
export default function ProvinceSelector() {
  const { changeProvince } = useProvince();
  ...
  return (
    <div className="flex items-center gap-1.5 text-sm">
      ...
      <select
        value="ON"
        onChange={() => changeProvince("ON")}
        aria-label="Delivery province"
      ...
      >
        <option value="ON" className="text-foreground bg-white text-base">Ontario (ON)</option>
      </select>
    </div>
  );
}
