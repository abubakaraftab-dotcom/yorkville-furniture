...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    // Only set visible if not explicitly closed
    if (sessionStorage.getItem("whatsapp_button_closed") !== "true") {
  ...
  const handleClose = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    sessionStorage.setItem("whatsapp_button_closed", "true");
    setIsVisible(false);
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <a
        ...
        className="flex items-center justify-center w-14 h-14 bg-success text-white rounded-full shadow-lg hover:bg-success/90 hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer group relative"
      ...
      <button
        onClick={handleClose}
        className="absolute -top-1 -right-1 flex items-center justify-center w-5 h-5 bg-gray-500 text-white rounded-full text-sm font-bold shadow hover:bg-gray-700 transition-colors z-10"
